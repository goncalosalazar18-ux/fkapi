...
class Command(BaseCommand):
    ...
    def _categorize(self, name):
        ...
        has_jacket_keyword = any(
            re.search(r"\b" + re.escape(keyword) + r"\b", normalized) for keyword in jacket_keywords
        )
    ...
    def handle(self, *args, **options):
        ...
        if not has_category:
            self.stdout.write(self.style.ERROR("Migration not applied. Please run: python manage.py migrate"))
            return
        ...
        for type_k in types:
            ...
            needs_update = (
                ...
                or type_k.category_order != category_order
                or type_k.is_goalkeeper != is_goalkeeper
                or type_k.order_priority != order_priority
            )
