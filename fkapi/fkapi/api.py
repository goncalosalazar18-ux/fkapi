...
from ninja import NinjaAPI, Query, Path
from ninja_apikey.security import APIKeyAuth
from ninja import Schema

# Local imports
from core.models import Club, Kit, Season, Brand, Competition
...
api = NinjaAPI(
    ...
    version="1.0.0",
    # auth=APIKeyAuth(),  # Temporarily disabled for local development
...
def get_seasons(
    request,
    id: int = Query(..., description="Club ID", example=1)
):
    """
...
def search_seasons(
    request,
    keyword: str = Query(..., description="Year or partial year to search for", example="2025")
):
    """
    ...
    if keyword.endswith('-'):
        ...
        if year_prefix.isdigit() and len(year_prefix) == 4:
            ...
            year_query = (
                Q(year__exact=f"{prev_year}-{year_short}") |
                Q(year__startswith=f"{year_int}-")
            )
        else:
...
def get_kit_json(
    request,
    kit_id: int = Path(..., description="Kit ID", example=1)
):
    """
    ...
    if kit.primary_color:
        primary_color = ColorJsonSchema(
            name=kit.primary_color.name,
            color=kit.primary_color.color
        )
    ...
    if kit.secondary_color.exists():
        secondary_colors = [
            ColorJsonSchema(name=color.name, color=color.color)
            for color in kit.secondary_color.all()
        ]
...
def send_kit(
    ...
    try:
        ...
        return JsonResponse(
            {"message": "Kit sent successfully", "response": response.json()}
        )
    except Kit.DoesNotExist:
        return JsonResponse({"error": "Kit not found"}, status=404)
    except requests.RequestException as e:
        return JsonResponse(
            {"error": f"Error sending data to the API: {str(e)}"}, status=500
        )
...
def search_kits(
    request,
    keyword: str = Query(None, description="Search query", example="Málaga 2003")
):
...
def test_search(request):
    ...
    for search_term, expected_match in test_cases:
        ...
        kits_with_similarity = all_kits.annotate(
        ...
        for kit in kits_with_similarity:
            kit_results.append({
                'id': kit.id,
                'name': kit.name,
                'similarity': float(kit.similarity),
                'team': kit.team.name
            })
...
def get_random_kits(request, page: int = 1, page_size: int = 20):
    ...
    for kit in page_obj:
        kits_data.append({
            ...
            'colors': f"{kit.primary_color.name} / {kit.secondary_color.name}" if kit.primary_color and kit.secondary_color else None,
            'design': kit.design,
        })
...
def get_merge_suggestions(request, threshold: float = 0.70, limit: int = 50):
    ...
    def normalize_name(name):
        ...
        prefixes = ['fc ', 'cf ', 'cfc ', 'sc ', 'ac ', 'ud ', 'cd ', 'sd ', 'rcd ', 'rc ', 
                   'real ', 'athletic ', 'atletico ', 'atlético ', 'deportivo ', 'sporting ',
                   'club ', 'football club ', 'futbol club ', 'soccer club ', 'tsv ', 'as ']
    ...
    def remove_exclusion_words(name):
        ...
        exclude_words = ['beleza', 'gloriosas', 'futsal', 'beach', 'indoor', 
                        'femenino', 'femenina', 'frauen', 'women', 'woman']
        words = name.split()
    ...
    def calculate_similarity(name1, name2, logo1, logo2):
        ...
        elif logo1 and logo2 and logo1 == logo2 and logo1 != 'https://www.footballkitarchive.com/static/logos/not_found.png':
            scores['logo_match'] = 0.3
        else:
    ...
    try:
        exclude_keywords = ['femenino', 'femenina', 'frauen', 'women', 'woman', 'sub-', 'sub ', 'sub19', 'sub-19', 'sub 19',
                          'sub20', 'sub-20', 'sub 20', 'sub21', 'sub-21', 'sub 21',
                          'filial', 'b team', 'b-team', 'reserve', 'reserves', 'youth',
                          'academy', 'u19', 'u20', 'u21', 'u23', 'u-19', 'u-20', 'u-21', 'u-23',
                          'beleza', 'gloriosas', 'futsal', 'beach', 'indoor']
        ...
        for i, club1 in enumerate(filtered_clubs):
            for club2 in filtered_clubs[i+1:]:
                ...
                similarity, scores = calculate_similarity(
                    club1.name, club2.name, club1.logo, club2.logo
                )
